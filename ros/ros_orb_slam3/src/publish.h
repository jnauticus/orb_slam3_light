...
class Publish
{
  private:
    ...
    tf2::Matrix3x3 _tf_orb_to_ros;

    tf2::TransformBroadcaster _tf_broadcaster;
    ros::Publisher _pose_pub;
    ros::Publisher _map_points_pub;
    // image_transport::Publisher _rendered_image_pub;

    tf2::Transform _from_orb_to_ros_tf_transform(const cv::Mat& transformation_mat)
    {
        ...
        tf2::Matrix3x3 tf_camera_rotation(orb_rotation.at<float>(0, 0), orb_rotation.at<float>(0, 1), orb_rotation.at<float>(0, 2), orb_rotation.at<float>(1, 0),
                                         orb_rotation.at<float>(1, 1), orb_rotation.at<float>(1, 2), orb_rotation.at<float>(2, 0), orb_rotation.at<float>(2, 1),
                                         orb_rotation.at<float>(2, 2));

        tf2::Vector3 tf_camera_translation(orb_translation.at<float>(0), orb_translation.at<float>(1), orb_translation.at<float>(2));

        // cout << setprecision(9) << "Rotation: " << endl << orb_rotation << endl;
        // cout << setprecision(9) << "Translation xyz: " << orb_translation.at<float> (0) << " " << orb_translation.at<float> (1) << " " << orb_translation.at<float> (2)
        // << endl;
    ...
    void _publish_tf_transform(const tf2::Transform& tf_transform, const rclcpp::Time& current_frame_time)
    {
        _tf_broadcaster.sendTransform(tf2::StampedTransform(tf_transform, current_frame_time, _map_frame_id, _pose_frame_id));
    }

    void _publish_pose_stamped(const tf2::Transform& tf_transform, const rclcpp::Time& current_frame_time)
    {
        tf2::Stamped<tf2::Pose> grasp_tf_pose(tf_transform, current_frame_time, _map_frame_id);
        geometry_msgs::PoseStamped pose_msg;
        tf2::poseStampedTFToMsg(grasp_tf_pose, pose_msg);
        _pose_pub.publish(pose_msg);
    }

    void _publish_tracked_mappoints(const std::vector<ORB_SLAM3::MapPoint*> map_points, const rclcpp::Time& current_frame_time)
    {
        ...
        for (unsigned int i = 0; i < cloud.width; i++)
        {
            if (map_points[i])
            {
                tf::Vector3 point_translation(map_points[i]->GetWorldPos().at<float>(0), map_points[i]->GetWorldPos().at<float>(1),
                                              map_points[i]->GetWorldPos().at<float>(2));
  ...
  public:
    Publish(cv::FileStorage& settings, ros::NodeHandle& node, ORB_SLAM3::System::eSensor sensor_type)
    {
        ...
        {
            _tf_orb_to_ros.setValue(0, 0, 1, -1, 0, 0, 0, -1, 0);
        }
        else if (sensor_type == ORB_SLAM3::System::IMU_MONOCULAR || sensor_type == ORB_SLAM3::System::IMU_STEREO)
        {
            _tf_orb_to_ros.setValue(0, 1, 0, -1, 0, 0, 0, 0, 1);
        }
        else
            _tf_orb_to_ros.setIdentity();
    }

    void publish(const cv::Mat& pos, const rclcpp::Time& current_frame_time, ORB_SLAM3::System& slam)
    {
        if (!pos.empty())
        {
